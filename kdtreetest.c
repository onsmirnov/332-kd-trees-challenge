...
int main(int argc, char **argv) {
  ...
  presults = kd_nearest_range( ptree, pt, testradius );
  int nresults = kd_res_size(presults);
  double fraction = (double)nresults/(double)num_pts;
  if (fraction > 0.31 && fraction < 0.34) {
